...
export const Wind4 = ({ bearing }) => {
  // create the particle variables
  const particleCount = 2000;
  const particleSize = 0.05;
  const particleTrailSize = 1;
  const particleTrailGap = 0.01;
  const particleSpeed = 0.007;
  const particleDepth = 3;
  const scale = 50;
  ...
  for (var p = 0; p < particleCount; p++) {
    const x = 0;
    const y = MathUtils.randFloatSpread(height) / 3;
    // const z = 0;
    const z = MathUtils.randFloatSpread(particleDepth);
    particles.push(x, y, z);
    for (let i = 0; i < particleTrailSize + 10; i++) {
      particles.push(x, y, z);
    }
    particleOptions.push({
      speed: particleSpeed + (0.5 - randomDecimal()) / 10000000,
      life: 0.5, // in seconds
    });
  }
